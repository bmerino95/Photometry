...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 24 12:08:31 2021
...
"""
2021/03/18 - This version of the code performs photometry over individual clumps - BMM
"""
...
counter =int(config.get('CONFIG','clump'))
...
path = '/Users/brianmerino/Desktop/Apperture_Test/%s-%s/clump_%s_phot.csv'%(cluster,ID,counter)
...
def phot(image_drz,noise,photflam,photplam):
    ...
    # This if statements corrects if the target is off of the detector in some filters
    #if flux_err == 0:
    #    mag = hst_phot(photflam, photplam, flux, dn_err=None) # AB magnitude
    #    magerr = -99
...
for item in filter_list:
    ...
    pf = fits.open(sub_img_dir_drz+'%s-%s/masked/clump_'%(cluster,ID)+str(counter)+'_'+item+'.fits')
    image = pf[ext].data
    head = pf[ext].header
    ...
    pf_wht = fits.open(sub_img_dir_drz+'%s-%s/masked/clump_'%(cluster,ID)+str(counter)+'_'+item+'_wht.fits')
    ...
    #mag[item+'_mag']=phot(image,photflam,photplam)
    #magerr[item+'_magerr']=-99
...
plt.savefig(plot_dir + '/%s-%s/clump_%s.png'%(cluster,ID,counter))
plt.show()
